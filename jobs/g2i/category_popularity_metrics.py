...
class CategoryPopularityMetricsJobs:
    ...
    def read_input_data(self):
        ...
        if not trending_path or not categories_path:
            raise ValueError(
                "Missing one of required dependencies: trending videos or categories.")
...
def main(spark_session: SparkSession, dataset_name: str, config_path: str):
    job = CategoryPopularityMetricsJobs(
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Category Popularity Metrics")
    ...
    logger.info(
        f"Starting CategoryPopularityMetricsJobs for {args.dataset_name}")
    try:
        spark_session = SparkSessionManager.get_session()
        main(spark_session, args.dataset_name, args.config_path)
    except Exception as e:
        logger.error(
            f"Error in CategoryPopularityMetricsJobs: {str(e)}", exc_info=True)
        raise
    finally:
